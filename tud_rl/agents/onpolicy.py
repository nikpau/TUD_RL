...
__currentmodule__ = import_module("tud_rl.agents.onpolicy")

modules = glob.glob(join(dirname(__path__[0] + "/_onpolicy/"), "*.py"))
_DAGENTS = []
for f in modules:
    if isfile(f) and not f.endswith('__init__.py'):
        _DAGENTS.append(basename(f)[:-3])

logger.info("--- Loading On-Policy Agents: ---")

for filename in _DAGENTS:
    module = import_module("tud_rl.agents._onpolicy." + filename)
